...
def prediction_de_langue(dico_test, models):  # Fonction pour prédire la langue d'un fichier de test
    ...
    for langue, ngramme_frequent in models.items():
        intersection = len(set(dico_test.keys()) & set(ngramme_frequent))
...
def process_test_files(models, n):  # Fonction pour traiter les fichiers de test et récupérer les prédictions
    predictions = {}
    liste_fichier_test = glob.glob(input)
    print(f"Nombre de fichiers de test : {len(liste_fichier_test)}")

    for chemin in liste_fichier_test:
        print(f"Traitement du fichier de test : {chemin}")

        # Obtenir la langue réelle à partir du chemin du fichier
        dossiers = chemin.split("\\")
        langue_reelle = dossiers[3]

        # Lire le fichier et extraire les n-grammes
        with open(chemin, "r", encoding="utf-8") as fichier:
            texte = fichier.read()
        ngrammes = extraire_ngrammes(texte, n)

        # Créer un dictionnaire des n-grammes les plus fréquents
        dico_test = dict(Counter(ngrammes).most_common(10))

        # Prédire la langue
        langue_predite = prediction_de_langue(dico_test, models)
...
# Fonction principale pour l'exercice 3 avec n-grammes
def main(n):
    models = open_file()
    predictions = process_test_files(models, n)
    exactitude, stats_par_langue = evaluer_statistiques(predictions)

    # Sauvegarder les résultats dans un fichier JSON
    with open(f"predictions_{n}-grammes.json", "w", encoding="utf-8") as fichier:
        json.dump(predictions, fichier, indent=2, ensure_ascii=False)
    print(f"Prédictions sauvegardées dans predictions_{n}-grammes.json")



for n in range(1,5): # Exécuter pour des valeurs de n de 1 à 5
    print(f"\n### Test avec {n}-grammes ###")
    main(n)
