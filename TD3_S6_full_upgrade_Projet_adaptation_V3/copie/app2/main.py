...
def print_cluster_stats(json_dir):
    ...
    for json_path in json_files:
        ...
        print(f"  Clusters contenant un seul élément   : {singleton_clusters}")
        print(f"  Moyenne de distance globale : {avg_distance:.4f}")
        print(f"  Intersections totales : {inter_size}")
        print(f"  Taille moyenne de cluster   : {avg_cluster_size:.2f}")
        print(f"  Taille minimum / maximum      : {min_cluster_size} / {max_cluster_size}")

        if n_clusters > 1:
            purity = max(cluster_sizes) / n_tokens
            print(f"  Approximation de pureté         : {purity:.3f}")
...
def plot_partition(words, ngram_range, title, out_path):
    ...
    ax.set_title(title + "\nÉchelonnement Multidimensionnel sur les similarités de ngrammes", fontsize=11)
    ax.set_xlabel("Dimension 1")
    ax.set_ylabel("Dimension 2")
...
def generate_ngram_partitionings(json_dir, output_dir):
    ...
    for json_path in collect_json_files(json_dir):
        ...
        for ngram_range, suffix in ngram_configs:
            title = f"{file_origin} - {suffix.replace('_', '/')}"
            fname = f"{base}_{suffix}.png"
            out_path = os.path.join(output_dir, fname)
            print(f"[PARTITION] {file_origin} → {fname}")
            plot_partition(tokens, ngram_range=ngram_range, title=title, out_path=out_path)
...
if __name__ == "__main__":
    ...
    for json_path in json_files:
        ...
        title = f"Échelonnement Multidimensionnel - {file_origin}"
